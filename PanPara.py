...
def get_opts():
    ...
    groups.add_argument('-l', '--list', help='list file, each row contain one sample name, first row means reference', required=True)
    groups.add_argument('-s', '--cds', help='cds directory, must exists all samples end with \".cds\", for example: \"sample1.cds\"', required=True)
    groups.add_argument('-b', '--bed', help='bed directory, must exists all samples end with \".bed\" , for example: \"sample1.bed\"', required=True)
    groups.add_argument('-d', '--iden', help="identity threshold, default=0.8", default=0.8, type=float)
    groups.add_argument('-c', '--coverage', help="the threshold of alignment coverage, default=0.8", default=0.8, type=float)
...
def PanPara(in_list, cds_dir, bed_dir, iden, cov, outdir, threads):
...
if __name__ == '__main__':
    ...
    outdir = opts.output
    threads = opts.thread
    PanPara(in_list, cds_dir, bed_dir, iden, cov, outdir, threads)
